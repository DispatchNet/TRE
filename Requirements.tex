...
The additional functionalities for authenticated users are described in \hyl{3.32}

\subsection{Account information}
Following \hyl{1.1} and \hyl{4.2}, in reference to \hyl{3.1}, at registration the system must require the user to provide username, email and password. information must satisfy the constraints indicated at \hyl{4.1}.
Registration can finally succeed after \hyl{4.5}. At end of successful operation the user is notified as described at \hyl{4.4}

\subsection{Account deletion}
Following \hyl{1.1} and \hyl{4.2}, in reference to \hyl{3.1}, the system must allow users to delete their accounts, as well as any other related information. At end of successful operation the user is notified
as described at \hyl{4.4}.

\subsection{Account information change}
Following \hyl{1.1} and \hyl{4.2}, in reference to \hyl{3.1}, the system must allow users to change their information (\hyl{3.2}).
The new information must satisfy the constraints indicated at \hyl{4.1}. At end of successful operation, the user should be notified as described in \hyl{4.4}.

\subsection{Password reset}
Following \hyl{1.1}, in reference to \hyl{3.1}, the system shall allow users to request a reset of their password and change it accordingly to \hyl{3.4}.
The user will be notified as described \hyl{4.4} with instructions on how to reset.

\subsection{Login}
Following \hyl{1.1} the system should allow \un{Anonymous Users} to login using their username and password defined at \hyl{3.2}.

\subsection{Logout}
Following \hyl{1.1} the system should allow non-\un{Anonymous Users} to logout, switching to the restricted \un{Anonymous User} status.
...
\begin{itemize}
	...
	Mainly, identifier, services provided, capacities and available classes (indicated in \hyl{3.23}).
...
Following \hyl{1.2}, in reference to \hyl{4.7}, the system must provide an interface containing ticket information described at \hyl{3.8}. 
It should be possible to quickly switch to \hyl{4.10}.

\subsection{Ticket purchase}
Following \hyl{1.2} in reference to \hyl{4.7}, the system should allow users to buy tickets. It should show a final confirmation interface with all the ticket information described at \hyl{3.8}. 
The ticked will be marked as purchased and displayed properly in the history at \hyl{3.12}.

\subsection{Ticket cancellation}
Following \hyl{1.2}, the system should allow the cancellation of purchased tickets. It should show a final confirmation interface with the ticket information described at \hyl{3.8}. The ticket will be marked as cancelled and displayed properly in the history at \hyl{3.12}.
At end of successful operation, the user is notified as described in \hyl{4.4}.

\subsection{Ticket history}
Following \hyl{1.2}, in reference to \hyl{3.7}, the system should provide an interface displaying purchased tickets with the dynamics dictated at \hyl{3.10} and cancelled tickets with the dynamics dictated at \hyl{3.11}.
...
\subsection{Ranking}
Following \hyl{1.3}, in reference to \hyl{3.13}, the system needs to be able to distinguish and rank multiple \bf{paths} over the network over several factors, including but not limited to:
\begin{itemize}
	\item Length.
...
Following \hyl{1.3}, in reference to \hyl{3.13} and \hyl{3.14}, the system needs to be able to treat two segments of separate train routes (\bf{natural} or \bf{transfer-dependent}) joined at a common valid arrival-into-departure station as a \bf{transfer-dependent route}, being hierarchically equivalent to a natural train route and carrying 
...
Following \hyl{1.3}, the system should not consider train routes or paths over the network that pass through nodes or by trains that the user is  not allowed to use or purchase tickets for, be that because of train retirement, expectation of exceeded capacity, or incompatible train type.
...
Following \hyl{1.4}, in reference to \hyl{3.17} and \hyl{3.18}, the system should be able to recognise and/or receive, and to provide to users when a train is expected to be late, delayed, cancelled, substituted or deviated.

\subsection{Inference}
Following \hyl{1.4}, in reference to \hyl{3.17} and \hyl{3.18}, if the system is aware of a delay, deviation, or cancellation of a train's route or of a departure or arrival time in station, it should automatically propagate that to all remaining stations on the affected train's route, estimating positive or negative delay caused by deviations using available data.

\subsection{Defining a service request}
Following \hyl{1.7}, in reference to \hyl{4.11}, \hyl{3.30}, and \hyl{3.31}, and depending on \hyl{3.22}, \hyl{3.23}, and \hyl{3.24}.
...
Following \hyl{1.7}, in reference to \hyl{3.21} and \hyl{3.22}, the system must allow \un{Companies} to define a service-type with their own set of service classes as well as their pricing model for passenger trains.
...
Following \hyl{1.6} and to ease scheduling, the system should be able to generate \bf{schedule graphs} displaying services as they move through a certain \bf{path} along the network.

\subsection{Defining a station}
Following \hyl{1.6} and depending on \hyl{3.29}, the system must allow \un{Network Managers} to register a \bf{station}, which is an extension of a \bf{junction}
...
Following \hyl{1.7} and depending on \hyl{3.21}, the system should allow the \un{Network Managers} to review requests, after that they may:
...
Following \hyl{1.6}, in reference to \hyl{3.24}, the system should allow modification and deletion 
...
Following \hyl{1.1} and \hyl{4.2}, the passwords related to \hyl{3.2} must be at least 8 characters, with at least one upper and one lower case letter and one digit.

\subsection{Email notification}
\it{Product - Usability requirement}

The system should notify the user when certain events happen. This should be implemented by sending an email to the address specified at registration, as indicated at \hyl{3.1}.
...
Following \hyl{4.2}, the system must provide users review of their stored information, through email notification, as indicated at \hyl{4.4}.

\subsection{Tickets information consistency}
\it{Product - Usability requirement}

Following \hyl{1.2}, the system must consistently show the same ticket information data, which is listed at \hyl{3.8}, in the functionalities described at \hyl{3.9} and \hyl{3.12}; in the latter there should also be added hour and date of purchase and a classification on the status, being either valid, expired, or cancelled.

\subsection{Process interconnection}
\it{Product - Usability requirement}

Following \hyl{1.5}, for all processes the user can initiate: all other processes that require a similar type of input as the user's process or could accept the user's process' output as input should have some easily accessible way to be used as such if relevant.

\subsection{Input minimization}
\it{Product - Usability requirement}

Following \hyl{1.5}, in reference to \hyl{4.8}, all operations of the system that accept user's input about information the system can provide should be capable of automatically obtaining such information from the system itself.
...
Following \hyl{1.3}, all path's displays should take at most five hundred milliseconds to load, and the computation of a \bf{path} along the network should take no more than one second on top of that. 
